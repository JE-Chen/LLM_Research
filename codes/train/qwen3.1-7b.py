...
OUTPUT_DIR = os.environ.get("OUTPUT_DIR", "./outputs-lora-qwen3-1.7b")
...
def format_example(example):
    # 將 think 和 answer 分開，think 只在 prompt，answer 為要訓練輸出
...
def tokenize(batch):
    ...
    for p_ids, a_ids in zip(tokenized_prompt["input_ids"], tokenized_answer["input_ids"]):
        # input 是 prompt + answer
        input_id = p_ids + a_ids + [tokenizer.eos_token_id]
        input_ids.append(input_id)

        # labels 在 prompt 部分用 -100 不計 loss，只對 answer 計算 loss
